...
class Group:
    ...
    def get_event(self, week_day, pair_number):
        for event in self.event_arr:
            if event.week_day == week_day and event.pair_number == pair_number:
                return event
        FreeEvent = namedtuple('FreeEvent', ['week_day', 'audience_number', 'building_name'])
        return FreeEvent('', '', '')
...
def read_excel_timetable(apps, schema_editor):
    """Читает эксель и создает группы по прочитанным данным"""
    ...
    InstituteGroupDayHistory = apps.get_model('main', 'InstituteGroupDayHistory')
    ...
    for timetable in timetables:
        for group in timetable.group_arr:
            InstituteGroup.objects.filter(name=group.name).delete()
            # InstituteGroupDayHistory.objects.filter(group__name=group.name).delete()
            # InstituteGroupDayHistory.objects.filter(group=None).delete()
            if len(InstituteGroup.objects.filter(name=group.name)) == 0:
                institute = Institute.objects.get(name="МФТИ")
                new_group = InstituteGroup.objects.create(
                    name=group.name,
                    description=f"Группа: {group.name}",
                    institute=institute,
                    institute_group_day_history=InstituteGroupDayHistory.objects.create(
                        week_pairs=[
                            [
                                [
                                    TIME_SLOT_DICT[i+1],                                    #  Время бронирования
                                    group.get_pair_name(WEEK_DAYS_DICT[j], i),              #  Название мероприятия
                                    group.get_event(WEEK_DAYS_DICT[j], i).audience_number,  #  Название аудитории
                                    group.get_event(WEEK_DAYS_DICT[j], i).building_name,    #  Название задния
                                    "Ok",                                                   #  Статус мероприятия
                                    WEEK_DAYS_DICT[j],                                      #  День недели
                                ]
                            for i in range(14)] for j in range(1,6)]))
                new_group.institute_group_day_history.group = new_group
                new_group.institute_group_day_history.group.save()
                new_group.institute_group_day_history.save()
...
class Migration(migrations.Migration):
    dependencies = [
        ('main', '0004_institutegroupdayhistory_and_more'),
    ]
